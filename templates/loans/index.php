 ...
 */
$rows       = $result['rows'];
$hasFilters = ($filters['q'] ?? '') !== '' || !empty($filters['status']) || !empty($filters['borrower_id'])
    || !empty($filters['from']) || !empty($filters['to']);
?>
<div class="page-head">
    <div>
        <h1>Loans &amp; hires</h1>
        <p class="muted"><?= number_format($result['total']) ?> loan<?= $result['total'] === 1 ? '' : 's' ?><?= $hasFilters ? ' matching your filters' : '' ?></p>
    </div>
    <div class="head-actions">
        <?php if (can('loans.create')): ?>
            <a class="btn" href="<?= e(url('/scan?mode=checkout')) ?>">Scan to check out</a>
            <a class="btn btn-primary" href="<?= e(url('/loans/checkout')) ?>">Check out</a>
        <?php endif; ?>
    </div>
</div>
...
<div class="stat-grid">
    <a class="stat-card stat-link <?= $summary['overdue'] > 0 ? 'stat-danger' : '' ?>"
       href="<?= e(url('/loans?status%5B%5D=Overdue')) ?>">
        <span class="stat-value"><?= (int) $summary['overdue'] ?></span>
        <span class="stat-label">Overdue</span>
    </a>
    <a class="stat-card stat-link stat-info" href="<?= e(url('/loans?status%5B%5D=Out')) ?>">
    ...
    <a class="stat-card stat-link <?= $summary['due_soon'] > 0 ? 'stat-warn' : '' ?>"
       href="<?= e(url('/loans?status%5B%5D=Out&sort=due')) ?>">
    ...
    <a class="stat-card stat-link" href="<?= e(url('/loans?status%5B%5D=Returned')) ?>">
        <span class="stat-value"><?= (int) $summary['returned_30'] ?></span>
        <span class="stat-label">Returned in 30 days</span>
    </a>
</div>

<form method="get" action="<?= e(url('/loans')) ?>" class="card filter-card">
    <div class="search-row">
        <label class="sr-only" for="q">Search loans</label>
        <input class="input input-search" type="search" id="q" name="q" enterkeyhint="search"
               placeholder="Search reference, asset, borrower…" value="<?= e($filters['q']) ?>">
    ...
    <details class="filter-details" <?= $hasFilters ? 'open' : '' ?>>
        ...
        <div class="filter-grid">
            <div class="field">
                ...
                <div class="check-row">
                    <?php foreach (Loan::STATUSES as $status): ?>
            ...
            <div class="field">
                <label class="label" for="borrower">Borrower</label>
                <select class="input" id="borrower" name="borrower">
                    <option value="">Anyone</option>
                    <?php foreach ($borrowers as $borrower): ?>
                        <option value="<?= (int) $borrower['id'] ?>" <?= (string) $filters['borrower_id'] === (string) $borrower['id'] ? 'selected' : '' ?>>
                            <?= e($borrower['name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="field">
                ...
                <select class="input" id="sort" name="sort">
                    ...
                    <option value="borrower" <?= $filters['sort'] === 'borrower' ? 'selected' : '' ?>>Borrower</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-actions">
            ...
            <a class="btn btn-ghost" href="<?= e(url('/loans')) ?>">Clear all</a>
        </div>
    </details>
</form>
...
<?php if ($rows === []): ?>
    <div class="card empty-state">
        <h2><?= $hasFilters ? 'Nothing matched' : 'No loans yet' ?></h2>
        <p class="muted">
            <?= $hasFilters ? 'Try fewer filters.' : 'Check an asset out and it will appear here.' ?>
        </p>
        <?php if (!$hasFilters && can('loans.create')): ?>
...
<?php else: ?>
    <div class="table-wrap">
        <table class="table">
            ...
            <?php foreach ($rows as $loan): ?>
                <tr class="<?= $loan['effective_status'] === 'Returned' ? 'row-muted' : '' ?>">
                    <td class="nowrap">
                        <span class="badge loan-<?= e(strtolower((string) $loan['effective_status'])) ?>">
                            <?= e($loan['effective_status']) ?>
                        </span>
                        <?php if (!empty($loan['reference'])): ?>
                            <div class="cell-sub mono"><?= e($loan['reference']) ?></div>
                        <?php endif; ?>
                    </td>
                    <td>
                        <a class="asset-link" href="<?= e(url('/assets/' . $loan['asset_id'])) ?>">
                            <span class="mono asset-tag"><?= e($loan['asset_tag']) ?></span>
                            <span class="asset-name"><?= e(str_limit((string) $loan['asset_name'], 36)) ?></span>
                        </a>
                    </td>
                    <td>
                        <a href="<?= e(url('/borrowers/' . $loan['borrower_id'])) ?>"><?= e($loan['borrower_name']) ?></a>
                        <?php if (!empty($loan['company_name']) && $loan['company_name'] !== $loan['borrower_name']): ?>
                            <div class="cell-sub"><?= e($loan['company_name']) ?></div>
                        <?php endif; ?>
                    </td>
                    <td class="nowrap"><?= e(format_date($loan['checked_out_at'])) ?></td>
                    <td class="nowrap">
                        <?= e(format_date($loan['due_back_date'])) ?>
                        <?php if ($loan['returned_at'] === null && $loan['days_until_due'] !== null): ?>
                        ...
                        <?php elseif ($loan['returned_at'] !== null): ?>
                            <div class="cell-sub">back <?= e(format_date($loan['returned_at'])) ?></div>
                        <?php endif; ?>
                    </td>
                    <td class="actions">
                        <a class="btn btn-sm" href="<?= e(url('/loans/' . $loan['id'])) ?>">Open</a>
                        <?php if ($loan['returned_at'] === null && can('loans.return')): ?>
    ...
    <?php if ($result['pages'] > 1): ?>
        <nav class="pagination" aria-label="Pages">
            <?php $base = url('/loans') . ($queryString !== '' ? '?' . $queryString . '&' : '?'); ?>
