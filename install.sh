...
UPLOAD_MAX_PDF_MB="${UPLOAD_MAX_PDF_MB:-25}"
...
Options
  ...
  --seed-demo          load the demo data as well as the real schema
...
Answers file keys
  ...
  ORGANISATION_NAME DB_NAME DB_USER DB_PASSWORD DB_HOST DB_PORT DB_ROOT_PASSWORD
  WEB_SERVER SERVER_NAME HTTP_PORT TLS_MODE TLS_CERT TLS_KEY MAKE_DEFAULT_SITE
  ADMIN_NAME ADMIN_EMAIL ADMIN_PASSWORD SEED_DEMO
  UPLOAD_MAX_PHOTO_MB UPLOAD_MAX_PDF_MB
...
case "$(uname -s)" in
    Linux) : ;;
    *) die "This installer targets Linux. On Windows or macOS, follow the manual steps in README.md." ;;
esac
...
if [ -n "$PHP_BIN" ]; then
    ...
    PLAN+=("Install PHP 8.1+ with pdo_mysql, mbstring, fileinfo, gd and exif")
fi
...
# even though public/index.php is already sitting in place.
...
say "  This account can do everything, including creating other users."
...
PLAN+=("Create the administrator $ADMIN_EMAIL")
[ "$SEED_DEMO" = yes ] && PLAN+=("Load the demo data")
...
install_packages() {
    ...
    case "$PKG" in
        ...
        dnf|yum)
            ...
            ;;
        zypper)
            pkgs+=(php8 php8-mysql php8-mbstring php8-gd php8-fileinfo)
...
missing=()
for ext in pdo pdo_mysql mbstring fileinfo json; do
    if "$PHP_BIN" -m | grep -qix "$ext"; then ok "$ext"; else missing+=("$ext"); warn "$ext is MISSING"; fi
done
for ext in gd exif; do
    if "$PHP_BIN" -m | grep -qix "$ext"; then ok "$ext (optional)"; else warn "$ext is missing — photos will not be resized or straightened"; fi
done
...
# usually already in. Fall back to a password, kept in a 600 defaults file so
...
case "$TLS_MODE" in
    proxy)        force_https=true;  trust_proxy=true  ;;
    direct-https) force_https=true;  trust_proxy=false ;;
    plain-http)   force_https=false; trust_proxy=false ;;
esac
...
UPLOAD_MAX_PDF_MB=${UPLOAD_MAX_PDF_MB}
...
post_max_size = $((UPLOAD_MAX_PDF_MB + 2))M
max_file_uploads = 20
memory_limit = 256M
...
server {
    # HTTP/2 is left off: the directive to enable it changed name in nginx
    # 1.25 and the old spelling warns on new builds. Add it once you know
    # which nginx this box has.
    listen 443 ssl;
    ${server_name_line}
    ...
    client_max_body_size ${UPLOAD_MAX_PDF_MB}m;
...
case "$WEB_SERVER" in
    ...
    none)   step "Web server"; warn "Skipped. Point a document root at $INSTALL_DIR/public — README.md has sample Apache and nginx config." ;;
esac
...
# ---------------------------------------------------------------------------
# 13. Migrations, administrator, demo data
# ---------------------------------------------------------------------------
...
if printf '%s' "$ADMIN_PASSWORD" | php_app bin/console.php user:create \
        --name="$ADMIN_NAME" --email="$ADMIN_EMAIL" --role=admin --stdin-password; then
...
if [ -n "$ORGANISATION_NAME" ]; then
    php_app bin/console.php setting:set --key=organisation_name --value="$ORGANISATION_NAME" >/dev/null \
        && ok "Organisation name set to '$ORGANISATION_NAME'"
fi

if [ "$SEED_DEMO" = yes ]; then
    step "Loading the demo data"
    warn "Demo accounts all use the password Workshop!Demo2026 — delete them before real use."
    php_app bin/seed.php --force || warn "The demo data could not be loaded."
fi
...
case "$TLS_MODE" in
    proxy)
        ...
        say "  or the application will redirect in a loop. README.md has a sample block."
        ;;
    plain-http)
...
if [ "$SEED_DEMO" = yes ]; then
    say ""
    say "  ${C_YELLOW}Demo data was loaded. Delete the example accounts before real use.${C_RESET}"
fi

say ""
say "  First things to do once you are signed in:"
say "    1. Settings — organisation name, asset tag prefix, the due-soon windows."
say "    2. Categories and Locations — add the ones you actually use."
say "    3. Print one barcode label at 100% scale and check it scans."
say ""
