 ...
 * making them editable would mean nine chances to produce a broken layout and
 * no way to improve all of them at once.
 *
 ...
 * a 600px body because that is what fits a preview pane. No media queries and
 * no web fonts — both are dead weight in a message this simple.
 */
final class Layout
{
    ...
    /** Is there a logo to embed? Decides whether the sender attaches one. */
    ...
    public static function wrap(string $content, string $subject = ''): string
    {
        ...
        $masthead = self::hasLogo()
            ? '<img src="cid:' . $cid . '" alt="' . $organisation . '" height="40"'
                . ' style="display:block;height:40px;width:auto;max-width:220px;border:0;outline:none;text-decoration:none;">'
            : '<span style="font-size:19px;font-weight:700;color:' . $ink . ';">' . $organisation . '</span>';
